...
class TestGardenViewSetRecommendAction(TestCase):
    def setUp(self):
        ...
        weather_patcher = patch(
            "backend.encouragemint.views.garden_viewset.get_garden_moisture", return_value="Medium")
        self.mock_weather = weather_patcher.start()
        self.addCleanup(weather_patcher.stop)

        trefle_patcher = patch("backend.encouragemint.views.garden_viewset.lookup_plants")
